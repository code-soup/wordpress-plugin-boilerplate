...
# Prompt the user to enter Plugin Name (requiCYAN)
read -rp "$(echo -e "${YELLOW}Plugin Name (requiCYAN):${NC} ")" plugin_name
...
echo -e "${CYAN}Plugin Name is [$plugin_name]${NC}"
...
echo -e "${CYAN}Plugin URI is [$plugin_uri]${NC}"

# Prompt the user to enter Description (requiCYAN)
read -rp "$(echo -e "${YELLOW}Description (A short description of the plugin. Keep this description to fewer than 140 characters.) (requiCYAN):${NC} ")" plugin_description

# Sanitize user input
plugin_description=$(echo "$plugin_description" | sed "s,\x1B\[[0-9;]*[a-zA-Z],,g")
echo -e "${CYAN}Plugin Descritpion is [$plugin_description]${NC}"
...
echo -e "${CYAN}Plugin Version is [$plugin_version]${NC}"
...
echo -e "${CYAN}Requires Wordpress is [$requires_wordpress]${NC}"

# Prompt the user to enter Requires PHP with a default value of 8.1
read -rp "$(echo -e "${GREEN}Requires PHP (The minimum requiCYAN PHP version) [default: 8.1]:${NC} ")" requires_php
...
echo -e "${CYAN}Requires PHP is [$requires_php]${NC}"
...
echo -e "${CYAN}Plugin author is [$plugin_author]${NC}"
...
echo -e "${CYAN}Plugin Author URI is [$plugin_author_uri]${NC}"
...
echo -e "${CYAN}Plugin License is [$plugin_license]${NC}"
...
echo -e "${CYAN}License URI is [$license_uri]${NC}"

# Prompt the user to enter Text Domain (requiCYAN)
read -rp "$(echo -e "${YELLOW}Text Domain (The gettext text domain of the plugin.) (requiCYAN):${NC} ")" plugin_textdomain

# Sanitize user input
plugin_textdomain=$(echo "$plugin_textdomain" | sed "s,\x1B\[[0-9;]*[a-zA-Z],,g")
echo -e "${CYAN}Plugin Text Domain is [$plugin_textdomain]${NC}"
...
read -rp "$(echo -e "${YELLOW}Enter PHP namespace (requiCYAN): ${NC} ")" php_namespace

# Default Value
php_namespace=$(printf '%q\n' "$php_namespace")
echo -e "${CYAN}PHP namespace is [$php_namespace]${NC}"
...
read -rp "$(echo -e "${YELLOW}Enter Plugin Prefix, don't add dash or underscore at the end (requiCYAN): ${NC} ")" plugin_prefix

# Default Value
plugin_prefix=$(printf '%q\n' "$plugin_prefix")
echo -e "${CYAN}PHP Prefix is [$plugin_prefix]${NC}"

while [[ -z "$plugin_prefix" ]]; do
    read -rp "$(echo -e "${YELLOW}Enter Plugin Prefix (requiCYAN): ${NC} ")" plugin_prefix
    plugin_prefix=$(printf '%q\n' "$plugin_prefix")
    echo -e "${CYAN}PHP Prefix is [$plugin_prefix]${NC}"
done
